...
(giống prod hiện tại) sang **GKE + Cloud SQL**, rồi dựng GitOps, giám sát, canary, xử lý sự cố.

## 1. Đã chốt

| Mục | Giá trị |
| --- | --- |
| Tài khoản | GCP Free Trial $300, kích hoạt **2026-09-25**, hết hạn khoảng **2026-12-24**. Không nâng lên tài khoản trả phí |
| Budget | $150, cảnh báo qua email ở 50% / 90% / 100% |
| Project | 1 project: `PROJECT_ID = ____________` (điền ở M0) |
...
- **Trước khi destroy GKE**, xoá các namespace có Ingress / Service LoadBalancer / PVC. Nếu không
  xoá, GCP sẽ để lại Load Balancer và disk nằm ngoài Terraform, vẫn tính tiền.
...
| Hostname | `app.` (app, chạy trên VM rồi chuyển sang GKE ở M6) · `demo.` (nginx ở M5) · `argocd.` · `grafana.` (tất cả dưới `gcp.zett.io.vn`) |
...
| Giám sát | kube-prometheus-stack (Prometheus, Grafana, Alertmanager → Discord), Loki + Alloy, Cloud Monitoring |
| Triển khai nâng cao | Argo Rollouts (canary + analysis) |
| Test tải | k6 |

Ingress dùng loại có sẵn của GKE (tạo Google Load Balancer + cert do Google quản lý).
Không dùng ingress-nginx vì dự án này đã ngừng phát triển từ 2026.
...
- [ ] 3.2 HTTPS Load Balancer + cert do Google quản lý + bản ghi `app.gcp.zett.io.vn`
...
- PV: Health check dùng để làm gì? Autoscale dựa trên cái gì? (Bên AWS tương ứng là ASG + ALB + CloudWatch)
...
### M6 · App lên GKE + Cloud SQL + chuyển DB (6h)
...
- [ ] 6.3 Tự viết Helm chart `charts/qltrungtam`: Deployment, Service, Ingress, Secret, Job migration (`prisma migrate deploy`)
- [ ] 6.4 Chuyển data: `pg_dump` từ `legacy-db` → restore vào Cloud SQL → kiểm tra bằng `npm run audit:data`
- [ ] 6.5 Cutover: dừng ghi → dump/restore lần cuối → trỏ `app.` sang Ingress của GKE, **đo downtime**
...
- [ ] 8.3 external-dns: tự tạo bản ghi trong zone `gcp.zett.io.vn` khi có Ingress mới
- [ ] 8.4 Chuyển toàn bộ add-on sang cài bằng Argo CD (app-of-apps)
- Xong khi: trong git không còn secret nào; tạo Ingress mới là DNS tự có bản ghi
...
Đầu buổi:
1. Chạy `gcloud config list` để chắc chắn đang đúng project.
